...
{
    internal sealed class IssuePropertyFileUploadToken : IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet("properties/{propertyId}/upload-token", async (
                Guid propertyId,
                string originalFileName,
                ISender sender) =>
            {
                var result = await sender.Send(new IssueFileUploadTokenCommand(propertyId, null, originalFileName));
